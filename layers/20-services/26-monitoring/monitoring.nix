  ...
  config = mkMerge [
    ...
    (mkIf config.layers.layer-20.services.config.monitoring.enable (
      let
        ...
        contractBlackboxScrapeConfigs = mapAttrsToList (name: svc: {
        ...
        services.prometheus = {
                  ...
                  rules = [
                    ...
                    }
                    {
                      ...
                      annotations = {
                        ...
                        description = "Service {{ $labels.service }} target {{ $labels.instance }} probe failed.";
                      };
                    }
                    {
                      alert = "FleetHealthcheckStalled";
                      expr = "node_systemd_unit_state{name=\"fleet-healthcheck.service\", state=\"active\"} == 0";
                      for = "15m";
                      labels = {
                        severity = "warning";
                      };
                      annotations = {
                        summary = "Fleet healthcheck runner stalled or inactive";
                        description = "fleet-healthcheck.service on {{ $labels.instance }} is not active.";
                      };
                    }
                  ];
                }
          ...
          scrapeConfigs = [
            ...
            }
            {
              job_name = "node-fleet";
              static_configs = [
                { targets = config.layers.layer-20.services.config.monitoring.fleetNodeTargets; }
              ];
            }
            {
              job_name = "postgres";
            ...
            }
            {
              ...
              static_configs = [
                {
                  targets = [
                    "localhost:${toString config.layers.layer-20.services.config.monitoring.loki.port}"
                  ];
                }
              ];
            }
            ...
            (mkIf (config.services.ai-services.kong-gateway.enable or false) {
              ...
              static_configs = [
                {
                  targets = [
                    "127.0.0.1:${toString config.services.ai-services.kong-gateway.proxyPort}"
                  ];
                }
              ];
            })
            ...
            (mkIf (config.services.ai-services.langgraph.enable or false) {
              ...
              static_configs = [
                {
                  targets = [
                    "127.0.0.1:${toString config.services.ai-services.langgraph.port}"
                  ];
                }
              ];
            })
