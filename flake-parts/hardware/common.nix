...
with lib;
{
  ...
  config = {
    ...
    boot.kernelPackages = mkMerge [
      ...
      (mkIf (config.hardware-config.kernel == "latest") pkgs.linuxPackages_latest)

      # CachyOS kernel - optimized for performance
      (mkIf (config.hardware-config.kernel == "cachyos") pkgs.linuxPackages_cachyos)

      # Zen kernel - optimized for desktop/responsiveness
      (mkIf (config.hardware-config.kernel == "zen") pkgs.linuxPackages_zen)
    ];
    ...
    systemd.user.services.udiskie =
      mkIf (config.hardware-config.automount.enable && config.hardware-config.automount.useUdiskie)
        {
    ...
    environment.systemPackages =
      (lib.optionals config.hardware-config.automount.enable (
        with pkgs;
        [
          udiskie
          udisks
